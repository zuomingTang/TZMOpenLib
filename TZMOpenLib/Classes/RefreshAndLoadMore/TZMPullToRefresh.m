...
@implementation UIScrollView (TZMPullToRefresh)
...
- (TZMPullToRefresh *)addPullToRefreshPosition:(TZMPullToRefreshPosition)position actionHandler:(void (^)(TZMPullToRefresh *v))handler
{
    ...
    for (UIView *v in self.subviews) {
        if ([v isKindOfClass:[TZMPullToRefresh class]])
            if (((TZMPullToRefresh *)v).position == position)
                return (TZMPullToRefresh *)v;
    }

    TZMPullToRefresh *view = [[TZMPullToRefresh alloc] initWithImage:[UIImage imageNamed:@"centerIcon"]
                                                          position:position];
    switch (view.position) {
        case TZMPullToRefreshPositionTop:
        case TZMPullToRefreshPositionBottom:
...
@interface TZMPullToRefreshBackgroundLayer : CALayer
...
@implementation TZMPullToRefreshBackgroundLayer
...
- (void)drawInContext:(CGContextRef)ctx
{
    ...
    //Draw circle outline
    CGContextSetStrokeColorWithColor(ctx, [UIColor colorWithWhite:1.0f alpha:0.9f].CGColor);
...
/*-----------------------------------------------------------------*/
@interface TZMPullToRefresh()

@property (nonatomic, assign) BOOL isUserAction;
@property (nonatomic, assign) TZMPullToRefreshState state;
@property (nonatomic, assign, readonly) BOOL isSidePosition;
@property (nonatomic, strong) TZMPullToRefreshBackgroundLayer *backgroundLayer;
...
@implementation TZMPullToRefresh
...
- (void)_commonInit
{
    ...
    self.state = TZMPullToRefreshStateNormal;
    ...
    if (self.dropDownType == TZMPullToRefreshDropDownTypeImage) {
        //init background layer
        TZMPullToRefreshBackgroundLayer *backgroundLayer = [[TZMPullToRefreshBackgroundLayer alloc] initWithBorderWidth:self.borderWidth];
        ...
        [self setSize:CGSizeMake(300, self.threshold)];
        self.label = [[UILabel alloc]initWithFrame:CGRectZero];
        self.label.font = [UIFont systemFontOfSize:16];
        self.label.textAlignment = NSTextAlignmentCenter;
        self.label.textColor = self.textColor ? self.textColor : [UIColor whiteColor];
...
- (void)setupScrollViewContentInsetForLoadingIndicator:(actionHandler)handler
{
    ...
    if (self.position == TZMPullToRefreshPositionTop) {
...
- (void)setProgress:(double)progress
{
    ...
    if (self.dropDownType == TZMPullToRefreshDropDownTypeText) {
        if (self.state == TZMPullToRefreshStateLoading) {
            [self refreshText:@"刷新数据中..."];
            
        }
        if (self.state == TZMPullToRefreshStateStopped) {
            [self refreshText:@"完成刷新"];
        }
        if (self.state == TZMPullToRefreshStateNormal) {
...
- (BOOL)isSidePosition
{
    return (self.position == TZMPullToRefreshPositionLeft || self.position == TZMPullToRefreshPositionRight);
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset
{
    ...
    if (self.scrollView != self.superview) {
        centerY = -centerY;
    }
    
    self.center = CGPointMake(centerX, centerY);
    switch (self.state) {
        case TZMPullToRefreshStateNormal: //detect action
            if (self.isUserAction && !self.scrollView.dragging && !self.scrollView.isZooming && self.prevProgress > 0.99f) {
                [self actionTriggeredState];
            }
            break;
        case TZMPullToRefreshStateStopped: // finish
        case TZMPullToRefreshStateLoading: // wait until stopIndicatorAnimation
            break;
        default:
            break;
    }
...
{
    self.state = TZMPullToRefreshStateLoading;
...
-(void)refreshText:(NSString*)text{
    ...
    CGFloat width = [self.label.text sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.label.font,NSFontAttributeName,nil]].width;
    self.label.frame = CGRectMake((self.bounds.size.width - width) / 2.0f, 0, width, self.bounds.size.height);
    self.activityIndicatorView.frame = CGRectMake(self.label.frame.origin.x - 40, 0, 40, self.bounds.size.height);
}
